...
namespace UltimateGameTools
{
    namespace MeshSimplifier
    {
        ...
        public class Simplifier : MonoBehaviour
        {
            ...
            public IEnumerator ProgressiveMesh(Mesh sourceMesh, RelevanceSphere[] aRelevanceSpheres, string strProgressDisplayObjectName = "", ProgressDelegate progress = null)
            {
                m_meshOriginal = sourceMesh;
                ...
                if (Application.isEditor && !Application.isPlaying)
                {
                    IEnumerator enumerator = ComputeAllEdgeCollapseCosts(strProgressDisplayObjectName, null, aRelevanceSpheres, progress);
                ...
                {
                    yield return StartCoroutine(ComputeAllEdgeCollapseCosts(strProgressDisplayObjectName, null, aRelevanceSpheres, progress));
                }
                ...
                while (m_listVertices.Count > 0)
                {
                    ...
                    m_listVertexPermutationBack[m_listVertices.Count - 1] = mn.m_nID;
                    m_listVertexMap[mn.m_nID] = mn.m_collapse != null ? mn.m_collapse.m_nID : -1;
                    Collapse(mn, mn.m_collapse, true, null, aRelevanceSpheres);
                }
            ...
            public IEnumerator ComputeMeshWithVertexCount(int nVertices, string strProgressDisplayObjectName = "", ProgressDelegate progress = null)
            {
                if (GetOriginalMeshUniqueVertexCount() == -1) //원본 매시의 버택스 카운트가 -1이면
                {
                    CoroutineEnded = true; //코루틴 종료
                    yield break;
                }

                if (nVertices < 3) //원본 매시의 버택스 카운트 * 비율이 3 보자 작으면 코루틴 종료
                {
                    CoroutineEnded = true; //코루틴 종료
                    yield break;
                }

                if (nVertices >= GetOriginalMeshUniqueVertexCount()) //변환 하는 매시의 버택스 카운트가 원본 매시의 버택스 카운트 보다 크다면?
                {
                    // Original vertex count requested  //원본으로 복귀
                     
                    m_meshSimplify.triangles = new int[0];
                    m_meshSimplify.subMeshCount = m_meshOriginal.subMeshCount;

                    m_meshSimplify.vertices = m_meshOriginal.vertices;
                    m_meshSimplify.normals = m_meshOriginal.normals;
                    m_meshSimplify.tangents = m_meshOriginal.tangents;
                    m_meshSimplify.uv = m_meshOriginal.uv;
                    m_meshSimplify.uv2 = m_meshOriginal.uv2;
                    m_meshSimplify.colors32 = m_meshOriginal.colors32;
                    m_meshSimplify.boneWeights = m_meshOriginal.boneWeights;
                    m_meshSimplify.bindposes = m_meshOriginal.bindposes;

                    m_meshSimplify.triangles = m_meshOriginal.triangles;
                    m_meshSimplify.subMeshCount = m_meshOriginal.subMeshCount;

                    for (int nSubMesh = 0; nSubMesh < m_meshOriginal.subMeshCount; nSubMesh++)
                    {
                        m_meshSimplify.SetTriangles(m_meshOriginal.GetTriangles(nSubMesh), nSubMesh);
                    }

                    m_meshSimplify.name = m_meshOriginal.name + " simplified mesh";
                ...
                if (Application.isEditor && !Application.isPlaying)
                {
                    IEnumerator enumerator = ConsolidateMesh(m_meshOriginal, m_aListTriangles, av3Vertices, strProgressDisplayObjectName, progress);
            ...
            {
                return m_meshSimplify;
            }

            public void SetSimplifyMesh(Mesh simplifyMesh)
            {
                m_meshSimplify = simplifyMesh;
            }
            ...
            IEnumerator ConsolidateMesh(Mesh meshIn, TriangleList[] aListTriangles, Vector3[] av3Vertices, string strProgressDisplayObjectName = "", ProgressDelegate progress = null)
            {
                Vector3[] av3NormalsIn = meshIn.normals;
                Vector4[] av4TangentsIn = meshIn.tangents;
                Vector2[] av2Mapping1In = meshIn.uv;
                ...
                m_meshSimplify.triangles = new int[0];
                m_meshSimplify.vertices = listVerticesOut.ToArray();
                m_meshSimplify.normals = listNormalsOut.Count > 0 ? listNormalsOut.ToArray() : null;
                m_meshSimplify.tangents = listTangentsOut.Count > 0 ? listTangentsOut.ToArray() : null;
                m_meshSimplify.uv = listMapping1Out.Count > 0 ? listMapping1Out.ToArray() : null;
                m_meshSimplify.uv2 = listMapping2Out.Count > 0 ? listMapping2Out.ToArray() : null;
                m_meshSimplify.colors32 = listColors32Out.Count > 0 ? listColors32Out.ToArray() : null;
                m_meshSimplify.boneWeights = listBoneWeightsOut.Count > 0 ? listBoneWeightsOut.ToArray() : null;
                m_meshSimplify.bindposes = meshIn.bindposes;
                m_meshSimplify.subMeshCount = listlistIndicesOut.Count;

                for (int nSubMesh = 0; nSubMesh < listlistIndicesOut.Count; nSubMesh++)
                {
                    m_meshSimplify.SetTriangles(listlistIndicesOut[nSubMesh].ToArray(), nSubMesh);
                }

                m_meshSimplify.name = meshIn.name + " simplified mesh";
